...
func _build_ui() -> void:
	...
	var title := Label.new()
	title.text = _default_status
	title.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
	title.add_theme_color_override("font_color", Color(0.94, 0.9, 0.82, 1))
	title.add_theme_font_size_override("font_size", 18)
	title.set_anchors_preset(Control.PRESET_TOP_WIDE)
	title.offset_left = 80
	title.offset_top = 112
	title.offset_right = -80
	title.offset_bottom = 148
	_status_label = title
	add_child(title)

	var scroll_panel := PanelContainer.new()
	scroll_panel.name = "ScrollPaper"
	scroll_panel.set_anchors_preset(Control.PRESET_TOP_WIDE)
	scroll_panel.offset_left = 32
	scroll_panel.offset_top = 128
	scroll_panel.offset_right = -32
	scroll_panel.offset_bottom = 392
	scroll_panel.add_theme_stylebox_override("panel", _paper_style())
	add_child(scroll_panel)

	var scroll_margin := MarginContainer.new()
	scroll_margin.add_theme_constant_override("margin_left", 28)
	scroll_margin.add_theme_constant_override("margin_top", 40)
	scroll_margin.add_theme_constant_override("margin_right", 28)
	scroll_margin.add_theme_constant_override("margin_bottom", 28)
...
func _drag_can_drop_row(row_id: String, _at: Vector2, data: Variant) -> bool:
	...
	# 填词阶段不按 filter_tag 限制拖放，与槽位3一致；仅在全部填满后统一判对错
