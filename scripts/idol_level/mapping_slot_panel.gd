...
func setup(slots: Array, vocab_bank: VocabBank) -> void:
	...
	for item in slots:
		...
		if str(row.get("slot_type", "")) == "mapping":
			_rows.append(row)
...
func _render_rows() -> void:
	...
	for row in _rows:
		...
		col.add_child(_build_icon_placeholder(row))
		var blank := SlotDrag.SlotBlankSlot.new()
		blank.setup_blank(self, row_id, Vector2(140, 40))
...
func _refresh_vocab_chips() -> void:
	...
	if _vocab_bank == null:
		return
	for vocab in _vocab_bank.get_collected_vocab("drink"):
...
func _build_icon_placeholder(row: Dictionary) -> PanelContainer:
	...
	icon_panel.add_theme_stylebox_override("panel", _icon_style(Color.from_string(color_text, Color(0.2, 0.5, 0.95, 1))))
	var icon_label := Label.new()
	icon_label.text = str(row.get("left_content", "图"))
...
func _icon_style(fill_color: Color) -> StyleBoxFlat:
	var radius := int(ICON_SIZE * 0.5)
	var style := StyleBoxFlat.new()
